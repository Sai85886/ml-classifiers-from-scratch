# Parkinson's voice data: binary. k-NN, Random Forest, NN.
import os
...
def run():
    ...
    print(f"[{DATASET_TAG}] Loaded {len(X)} samples, {X.shape[1]} features (binary labels).", flush=True)
    ...
    plot_metric_vs_param(knn_df, "k", "Parkinsons - KNN", out_dir, "knn", x_label="k (neighbors)")
    ...
    plot_metric_vs_param_grouped(
        rf_df, "n_trees", "criterion", "Parkinsons - Random Forest",
    ...
    plot_learning_curve(
        lc_df, f"Parkinsons - NN learning curve ({best_label})",
    ...
    pd.DataFrame(all_rows).to_csv(os.path.join(out_dir, "parkinsons_cv_results.csv"), index=False)
    print(f"\n[{DATASET_TAG}] Done. Saved → {out_dir}/parkinsons_cv_results.csv", flush=True)
